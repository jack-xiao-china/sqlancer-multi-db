...
| **GaussDB-A** | 24 | TLP_WHERE, NOREC, QUERY_PARTITIONING, HAVING, AGGREGATE, DISTINCT, GROUP_BY, PQS, CERT, DQP, DQE, EET, EET_INSERT_SELECT, EET_UPDATE, EET_DELETE, FUZZER, EDC_DATA, WRITE_CHECK, WRITE_CHECK_REPRODUCE, TX_INFER, FUCCI, JIR |
...
| **GaussDB-A** | 18 个 | 24 个 | EDC_DATA, EET_INSERT_SELECT, EET_UPDATE, EET_DELETE, TX_INFER, WRITE_CHECK, WRITE_CHECK_REPRODUCE |
...
#### 3.2 GaussDB-A 文档列出 2 个不存在的 Oracle
...
**影响位置** (英文 USER_GUIDE.md):
- Oracle Quick Reference 表格（第 62 行）
- GaussDB-A Oracle 详表（未列出 — 此处反而正确）

**影响位置** (中文 user_guide_cn.md):
- Oracle 快速参考表（第 82 行）

**修复**: 从 GaussDB-A 的 Quick Reference 行中删除 `EDC_RADAR` 和 `SONAR`。
