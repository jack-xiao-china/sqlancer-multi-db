...
- **25+ 种测试 Oracle**：支持 MySQL、PostgreSQL 和 GaussDB（包括 JIR、EDC、Sonar、EET、DQE、DQP、CODDTEST、QPG、FUCCI、TX_INFER）
...
| 数据库 | 可用的测试 Oracle |
|--------|-------------------|
| **MySQL** | TLP_WHERE, HAVING, GROUP_BY, AGGREGATE, DISTINCT, NOREC, QUERY_PARTITIONING, PQS, CERT, DQP, DQE, EET, EET_UPDATE, EET_DELETE, EET_INSERT_SELECT, CODDTEST, EDC, EDC_DATA, SONAR, WRITE_CHECK, WRITE_CHECK_REPRODUCE, FUCCI, TX_INFER, JIR, FUZZER |
| **PostgreSQL** | NOREC, PQS, TLP_WHERE, HAVING, AGGREGATE, DISTINCT, GROUP_BY, QUERY_PARTITIONING, CERT, DQP, DQE, EET, EET_UPDATE, EET_DELETE, EET_INSERT_SELECT, CODDTEST, EDC, EDC_DATA, SONAR, WRITE_CHECK, WRITE_CHECK_REPRODUCE, FUCCI, TX_INFER, JIR, FUZZER |
...
| **GaussDB-M** | TLP_WHERE, HAVING, GROUP_BY, AGGREGATE, DISTINCT, NOREC, QUERY_PARTITIONING, PQS, CERT, DQP, DQE, EET, EET_UPDATE, EET_DELETE, EET_INSERT_SELECT, CODDTEST, EDC, EDC_DATA, SONAR, WRITE_CHECK, WRITE_CHECK_REPRODUCE, FUCCI, TX_INFER, JIR, FUZZER |
...
java -jar sqlancer.jar gaussdb-m --target-database testm --oracle EDC --num-tries 100
```
...
| **JOIN 优化器 Bug** | JIR | EDC, DQP |
| **优化器覆盖率** | QPG | CERT, EET |
| **全面综合测试** | QUERY_PARTITIONING + JIR + EDC + EET + DQE + WRITE_CHECK | 所有其他 Oracle |
...
java -jar sqlancer.jar mysql \
    --oracle QUERY_PARTITIONING,DQE,DQP,EET,EDC,SONAR,JIR \
...
- **Oracle 支持更新**：EDC 和 SONAR 添加到所有支持的数据库
