...
- **25+ Test Oracles** for MySQL, PostgreSQL, and GaussDB (including JIR, EDC_RADAR, EDC_DATA, Sonar, EET, DQE, DQP, CODDTEST, FUCCI, TX_INFER)
...
## Version Highlights (v2.7.3–v2.7.9)

| Version | Key Changes |
|---------|-------------|
| **v2.7.9** | PostgreSQL non-superuser full-oracle sweep fix — `readFunctions` skips superuser/restricted builtins (`pg_replication_origin_*`/`pg_ls_*`/`pg_promote`/`pg_read_file`/etc.); EDC_DATA `func.txt` seed cleaned of 79 restricted functions; `COMMON_EXPECTED_SQLSTATES` extended with 8 states (`42701`/`42702`/`42710`/`23502`/`42830`/`42P04`/`55006`/`2BP01`) for non-English locale robustness; SONAR `getUnoptimizedQuery` handles `PostgresBinaryComparisonOperation`; unhandled types `IgnoreMeException`; disabled `FORCE ROW LEVEL SECURITY` (RLS 42501 root cause) |
| **v2.7.8** | PostgreSQL cloud-instance non-superuser fix — removed `pg_current_logfile()` and 6 SUSET GUC params (`wal_compression`/`commit_delay`/`track_activities`/`track_counts`/`track_io_timing`/`track_functions`) that raised `permission denied` under normal roles |
...
ls -lh target/sqlancer-2.7.9.jar
```
...
```bash
java -jar target/sqlancer-2.7.9.jar mysql --oracle QUERY_PARTITIONING
java -jar target/sqlancer-2.7.9.jar gaussdb-m --host xxx --port xxx --username xxx --password xxx
```

**Alternative** (use external classpath):
```bash
# Linux/macOS
java -cp "target/sqlancer-2.7.9.jar:target/lib/*" sqlancer.Main mysql --oracle NOREC
...
```bash
java -jar target/sqlancer-2.7.9.jar \
    --host localhost \
    --port 3306 \
    --username root \
...
| Type Category | Types Supported |
|---------------|-----------------|
| Numeric | NUMBER, NUMERIC, INTEGER, BIGINT, SMALLINT, TINYINT, FLOAT, REAL, DOUBLE PRECISION, DECIMAL |
| String | VARCHAR2, CHAR, TEXT, CLOB |
...
### EDC_RADAR (Equivalent Database Construction - RADAR)

**Concept**: EDC_RADAR detects optimizer bugs by comparing query results between two database states:
...
| **Optimizer regression testing** | EDC_RADAR, SONAR | NoREC, DQP |
| **JOIN optimizer bugs** | JIR | EDC_RADAR, DQP |
| **Performance regression** | CERT | NoREC |
| **Constraint correctness** | EDC_RADAR | DQE |
...
| **Cross-version testing** | DQP | EDC_RADAR |
...
- **New Documentation**: Comprehensive Oracle Reference Guide added
  ...
  - EDC_RADAR Oracle deep dive: Equivalent Database Construction concept and usage
...
- **Oracle Support Update**: EDC_RADAR and SONAR added to all supported DBMS tables
  - MySQL: 16 oracles (added EDC_RADAR, SONAR)
  - PostgreSQL: 16 oracles (added EDC_RADAR, SONAR)
  - GaussDB-M: 16 oracles (added EDC_RADAR, SONAR)
