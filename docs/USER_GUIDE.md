...
## Version Highlights (v2.7.3–v2.7.8)
...
ls -lh target/sqlancer-2.7.8.jar
```
...
```bash
java -jar target/sqlancer-2.7.8.jar mysql --oracle QUERY_PARTITIONING
java -jar target/sqlancer-2.7.8.jar gaussdb-m --host xxx --port xxx --username xxx --password xxx
```

**Alternative** (use external classpath):
```bash
# Linux/macOS
java -cp "target/sqlancer-2.7.8.jar:target/lib/*" sqlancer.Main mysql --oracle NOREC
