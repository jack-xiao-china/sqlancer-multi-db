...
public class MainOptions {
    ...
    @Parameter(names = "--connection-url", description = "Optional JDBC connection URL used by some DBMS providers (e.g. gaussdb). If unset, provider defaults apply.",
            arity = 1)
    private String connectionURL; // NOPMD

    @Parameter(names = "--jdbc-driver-class", description = "Optional JDBC driver class name to load explicitly (useful when DriverManager auto-loading does not work)",
            arity = 1)
    private String jdbcDriverClass; // NOPMD

    @Parameter(names = "--jdbc-properties", description = "Optional JDBC properties in k=v;k2=v2 format (provider may interpret)",
            arity = 1)
    private String jdbcProperties; // NOPMD

    @Parameter(names = "--use-create-database", description = "If supported, isolate each run by CREATE/DROP DATABASE; otherwise provider may fall back to schema-based isolation",
            arity = 1)
    private boolean useCreateDatabase = false; // NOPMD
    ...
    public boolean useCreateDatabase() {
        return useCreateDatabase;
    }
