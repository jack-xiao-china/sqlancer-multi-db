...
/**
 * GaussDB-M EET UPDATE Oracle — tests UPDATE by comparing row-state changes
 * between original and EET-transformed WHERE conditions.
 */
public class GaussDBMEETUpdateOracle
    ...
    public GaussDBMEETUpdateOracle(GaussDBMGlobalState state) {
        super(state, null);
    }
    ...
    protected SQLQueryAdapter generateDML(GaussDBMGlobalState state, Object gen,
        ...
        String whereStr = GaussDBToStringVisitor.asString(whereCondition);

        expressionGen.setLastGeneratedExpression(whereCondition);

        List<GaussDBColumn> columns = targetTable.getRandomNonEmptyColumnSubset();
        ...
        for (int i = 0; i < columns.size(); i++) {
            ...
            sb.append(" = ");
            sb.append(GaussDBToStringVisitor.asString(expressionGen.generateExpression()));
        }
        sb.append(" WHERE ");
        sb.append(whereStr);
