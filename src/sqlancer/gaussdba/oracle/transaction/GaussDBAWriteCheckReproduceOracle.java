...
public class GaussDBAWriteCheckReproduceOracle extends GaussDBAWriteCheckOracle {

    public GaussDBAWriteCheckReproduceOracle(GaussDBAGlobalState state) {
        super(state);
    }
    ...
    public void check() throws SQLException {
        ...
        try {
            for (GaussDBAIsolationLevel level : GaussDBAIsolationLevel.values()) {
                GaussDBATxTestExecutor testExecutor = new GaussDBATxTestExecutor(state, transactions, schedule, level);
                ...
                GaussDBATxTestExecutor oracleExecutor = new GaussDBATxTestExecutor(state, transactions, oracleSchedule, level);
                ...
                if (level == GaussDBAIsolationLevel.SERIALIZABLE) {
    ...
    private Transaction readTransactionFromScanner(Scanner input) throws SQLException {
        Transaction transaction = new Transaction(GaussDBATransactionProvider.createNewConnection(state));
        ...
        do {
            ...
            TxStatement cell = new GaussDBATxStatement(transaction, txStatement);
