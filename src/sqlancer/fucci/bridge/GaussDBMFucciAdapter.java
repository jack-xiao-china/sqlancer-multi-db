 ...
 * GaussDB-M Fucci适配器。
 */
public class GaussDBMFucciAdapter implements DBMSFucciAdapter {

    @Override
    public String getDbmsType() { return "gaussdbm"; }
    ...
    public boolean isDeadlockError(String errorMessage) {
        ...
        return errorMessage.contains("Deadlock") || errorMessage.contains("Lock deadlock");
    }

    @Override
    public boolean isLockWaitTimeoutError(String errorMessage) {
        if (errorMessage == null) { return false; }
        return errorMessage.contains("Lock wait timeout");
    }
