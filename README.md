...
**Version**: v2.7.9 (2026-06-24) · [Release Notes](docs/release_notes.md) · [Full User Guide](docs/USER_GUIDE.md) · [中文指南](docs/USER_GUIDE_CN.md)
...
# Output: target/sqlancer-2.7.9.jar (~4MB, deps in target/lib/)
```
...
| **4. FULL_JOIN_DECOMPOSITION** | `L FULL JOIN R ON c` = INNER + LEFT_ANTI + RIGHT_ANTI (full column coverage) | PG / GaussDB-A only |
| **5. CROSS_JOIN_EQUIVALENCE** | `L CROSS JOIN R` ≡ `L INNER/LEFT/RIGHT/FULL JOIN R ON TRUE` | 4 equivalence variants |
| **6. NATURAL_JOIN_EXPLICATION** | `NATURAL JOIN` ≡ `INNER JOIN ON (matching columns)` | Explicit equijoin condition |

```bash
java -jar target/sqlancer-2.7.9.jar --host localhost --port 3306 \
...
| **JIR Multi-Table JOIN** | 2-3 table JOIN chains, CROSS 4 variants (INNER/LEFT/RIGHT/FULL ON TRUE), Rule 4 full-anti coverage |
...
| **Schema Generation Robustness** | PostgresIndex `backsConstraint` check prevents invalid `ADD CONSTRAINT USING INDEX` |
...
```bash
java -jar target/sqlancer-2.7.9.jar --seed <seed_value> \
    --host localhost --port 3306 --username root --password your_password \
