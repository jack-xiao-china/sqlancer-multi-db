param(
  ...
  [string]$Username = "sqlancer",
  [string]$Password = "sqlancer",
  [switch]$UseCreateDatabase,
...
function Classify-RunResult([string]$out, [int]$exitCode) {
  ...
  # DB/environment failures (allowed for this smoke, but reported).
  $dbFailurePatterns = @(
...
if ([string]::IsNullOrWhiteSpace($ConnectionUrl)) {
  Write-Host "ERROR: -ConnectionUrl is required. Example: -ConnectionUrl `"jdbc:gaussdb://localhost:8000/test`""
  exit 2
}

# Order aligned with MySQL / GaussDBMOracleFactory enum (QUERY_PARTITIONING last).
$oracles = @(
  "AGGREGATE",
  "HAVING",
  "GROUP_BY",
...
$results = @()
$summaryPath = Join-Path $LogBaseDir "gaussdb-oracles-smoke-summary.txt"
Remove-Item -Force -ErrorAction SilentlyContinue $summaryPath

foreach ($o in $oracles) {
  Write-Host "`n==== Running GaussDB oracle $o ===="
  $args = @(
    "-jar", $jarFull,
  ...
  if (-not [string]::IsNullOrWhiteSpace($JdbcDriverClass)) {
    $args += @("--jdbc-driver-class", $JdbcDriverClass)
  }
  if (-not [string]::IsNullOrWhiteSpace($JdbcProperties)) {
    $args += @("--jdbc-properties", $JdbcProperties)
  }
  if ($UseCreateDatabase.IsPresent) {
    $args += @("--use-create-database", "true")
  }
...
$results | Export-Csv -NoTypeInformation -Encoding UTF8 (Join-Path $LogBaseDir "gaussdb-oracles-smoke-results.csv")
