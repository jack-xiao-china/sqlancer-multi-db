...
NUM_TRIES=10        # 默认每个 oracle 10 次尝试
...
# GaussDB-A: 25 oracles (v2.7.1+ 已支持 CODDTEST/EDC_RADAR/SONAR)
GAUSSDBA_ORACLES="AGGREGATE CERT CODDTEST DISTINCT DQE DQP EDC_DATA EDC_RADAR EET EET_DELETE EET_INSERT_SELECT EET_UPDATE FUZZER GROUP_BY HAVING JIR NOREC PQS QUERY_PARTITIONING SONAR TLP_WHERE"
...
选项:
  ...
  --num-tries N         每个 Oracle 的尝试次数 (默认: 10)
